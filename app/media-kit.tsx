...
import type { SponsorConfig, PricingConfig } from "./sponsors";
...
function PricingTable({ pricing }: { pricing: PricingConfig }) {
  const { tiers } = pricing;
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-16">
      <SectionHeader
        ...
        subtitle="Annual commitment, billed monthly or yearly"
      />

      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
        {tiers.map((tier, i) => {
          const isGold = i === tiers.length - 1;
          const isSilver = i === tiers.length - 2;
          ...
          return (
            <div
              ...
              className={`rounded-2xl overflow-hidden ${
                isGold
                  ? "bg-neutral-900 text-white ring-2 ring-neutral-900 md:scale-[1.03] origin-top"
                  : "bg-white border border-neutral-200/60 shadow-sm"
              }`}
            >
              {/* Header */}
              <div className={`p-6 pb-5 ${isGold ? "" : "border-b border-neutral-100"}`}>
                {tier.badge && (
                ...
                <p className={`text-xs font-semibold uppercase tracking-wider ${
                  isGold ? "text-neutral-400" : "text-neutral-400"
                } ${tier.badge ? "" : "mt-1"}`}>
                  {tier.name}
                </p>
                <p className={`text-3xl font-bold tracking-tight mt-1 ${isGold ? "text-white" : "text-neutral-900"}`}>
                  {tier.monthlyPrice}
                  <span className={`text-sm font-normal ${isGold ? "text-neutral-400" : "text-neutral-500"}`}> /mo</span>
                </p>
                <p className={`text-xs mt-1 ${isGold ? "text-neutral-500" : "text-neutral-400"}`}>
              ...
              {/* Feature sections */}
              <div className={`px-6 pb-6 ${isGold ? "pt-5 border-t border-white/10" : "pt-5"}`}>
                {tier.sections.map((section, sIdx) => (
                  <div key={section.label} className={sIdx > 0 ? "mt-5" : ""}>
                    <p className={`text-[10px] font-semibold uppercase tracking-wider mb-2.5 ${
                      isGold ? "text-neutral-500" : "text-neutral-400"
                    }`}>
                      {section.label}
                    </p>
                    <ul className="space-y-2">
                      {section.features.map((f) => (
                        <li key={f.text} className="flex items-start gap-2">
                          ...
                          <div>
                            <p className={`text-sm leading-snug ${isGold ? "text-white/90" : "text-neutral-800"}`}>
                              {f.text}
                            </p>
                            {f.note && (
                              <p className={`text-[11px] leading-snug mt-0.5 ${isGold ? "text-neutral-500" : "text-neutral-400"}`}>
                                {f.note}
                              </p>
                            )}
                          </div>
      ...
      <div className="text-center mt-12">
        <a
          href="mailto:info@openresidency.com?subject=Partnership Inquiry"
        ...
        <p className="text-xs text-neutral-400 mt-2">info@openresidency.com</p>
      </div>
    </section>
  );
}
...
export function MediaKitContent({ sponsorConfig }: { sponsorConfig?: SponsorConfig }) {
  ...
  const partners = ALL_PARTNERS.filter(
    (p) => !sponsorConfig?.hiddenPartners?.includes(p.name)
  );
    ...
    <main className="min-h-screen">
      ...
      <section className="max-w-6xl mx-auto px-6 pt-14 md:pt-20 pb-12">
        <div className="mb-14">
          ...
          <p className="text-lg text-neutral-500 leading-relaxed">
            {sponsorConfig?.heroTagline ?? "1.8M+ views in 6 months, and we\u2019re just getting started."}
          </p>
          <div className="flex flex-wrap gap-3 mt-6">
            ...
            <a href="https://www.instagram.com/openresidency" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-neutral-200 text-neutral-700 text-sm font-medium hover:bg-neutral-50 transition-colors">
      ...
      <section className="max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="space-y-5">
          ...
          <p className="text-neutral-600 leading-relaxed">
            {sponsorConfig?.customAboutText ?? "A partnership with Open Residency is not just media placement\u00a0\u2014 it\u2019s a relationship with Mark and his network as a business development and sales machine."}
          </p>
        </div>
      ...
      {/* ── LONG-FORM CONTENT ────────────────────────────────────── */}
      {!sponsorConfig?.hideSections?.includes("longForm") && (
      <section className="max-w-6xl mx-auto px-6 py-16">
        <SectionHeader
      ...
      {/* ── SHORT-FORM ──────────────────────────────────────────── */}
      {!sponsorConfig?.hideSections?.includes("shortForm") && (
      <section className="max-w-6xl mx-auto px-6 py-16">
        <SectionHeader
      ...
      {/* ── GUEST ROSTER ──────────────────────────────────────────── */}
      {!sponsorConfig?.hideSections?.includes("guestRoster") && (
      <section className="max-w-6xl mx-auto px-6 py-16">
        <SectionHeader
          title="Guest Roster"
      ...
      {!sponsorConfig?.hideSections?.includes("partners") && partners.length > 0 && (
      <section className="max-w-6xl mx-auto px-6 py-16">
        <SectionHeader
          title="Our Partners"
        ...
        <div className="bg-white rounded-2xl border border-neutral-200/60 shadow-sm p-8 md:p-10 mb-10">
          <div className={`grid grid-cols-2 gap-6 items-center ${partners.length === 3 ? "md:grid-cols-3" : "md:grid-cols-4"}`}>
            {partners.map((p) => (
      ...
      {/* ── PRICING ──────────────────────────────────────────────────── */}
      {sponsorConfig?.pricing && (
        <>
          <div className="max-w-6xl mx-auto px-6">
            <hr className="border-neutral-200" />
          </div>
          <PricingTable pricing={sponsorConfig.pricing} />
        </>
      )}

      {/* ── PHOTO GRID ──────────────────────────────────────────────── */}
      {!sponsorConfig?.hideSections?.includes("photos") && (
      <section className="max-w-6xl mx-auto px-6 py-16">
